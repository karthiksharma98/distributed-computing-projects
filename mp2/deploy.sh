...
for val in fa20-cs425-g13-0{1..9}.cs.illinois.edu; do
   ...
   then
       ...
       ssh $1@$val -t "cd cs425_mps && git fetch && git checkout $2 && cd mp2/src && rm -f machine.log.txt && git pull && rm -f machine.log.txt && bash ../build.sh && ls -l"
   fi
done
...
then
    ...
    ssh $1@fa20-cs425-g13-10.cs.illinois.edu -t "cd cs425_mps && git fetch && git checkout $2 && cd mp2/src && rm -f machine.log.txt && git pull && rm -f machine.log.txt && bash ../build.sh && ls -l"
fi