...
<<<<<<< HEAD
       ssh $1@$val -t "cd cs425_mps && git fetch && git checkout $2 && git pull && cd src && go build -o main && ls -l"
=======
       ssh $1@$val -t "cd cs425_mps && git fetch && git checkout $2 && git pull && cd src && rm machine.log.txt && go build -o main && ls -l"
>>>>>>> james/cleanupfix
   fi
done
